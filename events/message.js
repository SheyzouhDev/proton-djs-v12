const Discord = require ('discord.js'), db = require ('quick.db'), emoji = require ('../emoji.json'), cooldowns = new Discord.Collection()

module.exports = async (client, message) => {

    if (message.author.bot || message.author === client.user) return;
    if (message.channel.type === "dm") {
        const dmProton = '865791707035336764';
        client.channels.cache.get(dmProton).send(new Discord.MessageEmbed()
    ...
    if (!timestamps.has(member.id)) {
        if (!client.settings.owners.includes(message.author.id)) {
